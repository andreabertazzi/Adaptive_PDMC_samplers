...
function constructLogisticEnergy(Y::Matrix{Float64},Z::Vector{Int64}, prior_precision::Float64 = 1.0)
    ...
    n(x) = sqrt(dot(x,x))
    E(x) = sum(log.(1 .+ η(x))-Z' .* μ(x)) + 0.5*prior_precision * n(x)^2 # sum over observations
    dim = size(Y)[1]
    ∇E = function(x::Vector{Float64})
        η_val = η(x)
        sum(Y .* (η_val ./(1 .+ η_val) - Z'), dims=2)[:,1] + prior_precision * x # sum over observations
        # what this does is multiply i-th column of Y with the i-th component of the row vector
        # the i-th column corresponds to the i-th observation. Then sum over observations. OK.
    end
    h_E = function(x::Vector{Float64}) #TO CHECK
        η_val = η(x)
    ...
    partials_E = function(i::Int,x::Vector{Float64})
        η_val = η(x)
        dim = length(x)
        sum(transpose(view(Y, i, :)) .* (η_val ./(1 .+ η_val) - Z')) + prior_precision*x[i]   # MISSING prior_precision in front x[i]?
        # isolate i-th component: components are rows, observations columns.
...
function LogisticSubsamplingTools(Y::Matrix{Float64},Z::Vector{Int64}, prior_precision::Float64 = 1.0)
    μ(j, x) = sum(x .* view(Y,:,j)) # sum over the parameter dimensions ## isolate j-th observation
    η(j, x) = exp(μ(j, x))
    n = length(Z)  # number of observations
    dim = size(Y)[1]  # dimensionality
    partials_ss = function(i::Int,j::Int,x::Vector{Float64})   # choose both component and data-point
        η_val = η(j,x)
        n * Y[i,j] * (η_val/(1 + η_val)-Z[j]) + prior_precision * x[i]   # correct that there is no n in front of x[i]
    end
    Q_ss = LinearAlgebra.symmetric(n/4 * [maximum(abs.(view(Y,i,:) .* view(Y,j,:))) for i = 1:dim, j = 1:dim],:U)
    ∇E_ss = function(j::Int,x::Vector{Float64})   # choose data point only, get gradient
        η_val = η(j,x)
...
function generateLogisticData(parameter::Vector{Float64}, n_observations::Int)
  ...
  q = exp.(sum(parameter .* X,dims=1))  # create lines for each observation for the given parameter
  p = q./(1 .+ q)                       # probabilities of the output
  U = rand(n_observations)
  Y = [Int(U[i] <= p[i]) for i in 1:n_observations]   # create labels with the correct probability
  return (X,Y)
end
